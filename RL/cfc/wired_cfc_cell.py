...
class WiredCfCCell(nn.Module):
    def __init__(
        ...
        self.logger = TensorLogger("./RL/data/", slots=["state", "sensory", "command", "output"])
    ...
    def forward(self, input, hx, timespans):
        ...
        for i in range(self.num_layers):
            ...
            if input.shape[0] == 1:
                if i == 0:
                    self.logger.append(state=input)
                    self.logger.append(sensory=h)
                elif i == 1:
                    self.logger.append(command=h)
                elif i == 2:
                    self.logger.append(output=h)
